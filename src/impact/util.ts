...
export function rangeMap(n: number, istart: number, istop: number, ostart: number, ostop: number): number {
  return ostart + (ostop - ostart) * ((n - istart) / (istop - istart));
}

export function inject(Class: any, mixin: Record<string, any>): void {
