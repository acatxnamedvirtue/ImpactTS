...
export class igFont extends igImage {
  ...
  widthForString(text: string): number {
    // Multiline?
    if (text.indexOf("\n") !== -1) {
  ...
  _widthForLine(text: string): number {
    let width = 0;
  ...
  heightForString(text: string): number {
    return text.split("\n").length * (this.height + this.lineSpacing);
  }

  draw(text: any, x: number, y: number, align: igFontAlign = igFontAlign.LEFT): void {
  ...
  private _drawChar(c: number, targetX: number, targetY: number): number {
  ...
  _loadMetrics(image: HTMLImageElement | HTMLCanvasElement): void {
