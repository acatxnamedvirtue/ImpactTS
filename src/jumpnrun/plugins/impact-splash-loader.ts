...
import { rangeMap } from "../../impact/util";

export class ImpactSplashLoader extends igLoader {
  static OPS = {
    bp: "beginPath",
    cp: "closePath",
    f: "fill",
  ...
  run(): void {
    const t = Date.now() - this.endTime;
    let alpha = 1;
    if (t < this.fadeToWhiteTime) {
      // Draw the logo -> fade to white
      this.draw();
      alpha = rangeMap(t, 0, this.fadeToWhiteTime, 0, 1);
    } else if (t < this.fadeToGameTime) {
      // Draw the game -> fade from white
      ig.game.run();
      alpha = rangeMap(t, this.fadeToWhiteTime, this.fadeToGameTime, 1, 0);
    } else {
  ...
  draw(): void {
    ...
    // Draw 'impact' text
    this.drawPaths("rgb(255,255,255)", ImpactSplashLoader.PATHS_IMPACT);

    // Some quick and dirty hackery to make the comet's tail wiggle
    const comet = ImpactSplashLoader.PATHS_COMET as any;
  ...
  drawPaths(color: string, paths: Array<string | number[]>): void {
    const ctx = ig.system.context;
    ctx.fillStyle = color;
    ...
    for (let i = 0; i < paths.length; i += 2) {
      ...
      ctx[ImpactSplashLoader.OPS[paths[i] as string]].apply(ctx, paths[i + 1]);
    }
  }
}
